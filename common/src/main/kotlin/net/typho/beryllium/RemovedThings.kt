...
object RemovedThings {
    ...
    fun reload() {
        ...
        if (!ModConfig.instance.removedItems.vanillaTieredGearEnabled) {
            items += listOf(
                // keep wooden tools
        ...
        if (!ModConfig.instance.removedItems.maceEnabled) {
            items += Items.MACE
        }

        if (!ModConfig.instance.crossbows.burstCrossbow.enabled) {
            items += ModItems.burstCrossbow.get()
        }
    }
