...
public class PointedBoneBlock extends Block implements LandingBlock, Waterloggable {
    ...
    @Override
    public MapCodec<PointedBoneBlock> getCodec() {
        return CODEC;
    }

    public PointedBoneBlock(AbstractBlock.Settings settings) {
        super(settings);
    ...
    protected void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        if (random.nextFloat() < 0.011377778F && isHeldByPointedBone(state, world, pos)) {
    ...
    public static void dripTick(BlockState state, ServerWorld world, BlockPos pos, float dripChance) {
        if (!(dripChance > 0.17578125F) || !(dripChance > 0.05859375F)) {
            if (isHeldByPointedBone(state, world, pos)) {
                Optional<PointedBoneBlock.DrippingFluid> optional = getFluid(world, pos, state);
    ...
    private static Thickness getThickness(WorldView world, BlockPos pos, Direction direction, boolean tryMerge) {
        ...
        if (isPointedBoneFacingDirection(blockState, direction2)) {
            return !tryMerge && blockState.get(THICKNESS) != Thickness.TIP_MERGE ? Thickness.TIP : Thickness.TIP_MERGE;
        } else if (!isPointedBoneFacingDirection(blockState, direction)) {
            ...
            if (thickness != Thickness.TIP && thickness != Thickness.TIP_MERGE) {
                ...
                return !isPointedBoneFacingDirection(blockState2, direction) ? Thickness.BASE : Thickness.MIDDLE;
            } else {
    ...
    private static boolean canPlaceAtWithDirection(WorldView world, BlockPos pos, Direction direction) {
        ...
        BlockState blockState = world.getBlockState(blockPos);
        return blockState.isSideSolidFullSquare(world, blockPos, direction) || isPointedBoneFacingDirection(blockState, direction);
    }
    ...
    private static boolean isPointingDown(BlockState state) {
        return isPointedBoneFacingDirection(state, Direction.DOWN);
    }

    private static boolean isPointingUp(BlockState state) {
        return isPointedBoneFacingDirection(state, Direction.UP);
    }
    ...
    public static BlockPos getDripPos(World world, BlockPos pos) {
        BiPredicate<BlockPos, BlockState> biPredicate = (posx, state) -> canDripThrough(world, posx, state);
        return searchInDirection(world, pos, Direction.UP.getDirection(), biPredicate, PointedBoneBlock::canDrip, 11).orElse(null);
    }

    public static Fluid getDripFluid(ServerWorld world, BlockPos pos) {
        return getFluid(world, pos, world.getBlockState(pos)).map(fluid -> fluid.fluid).filter(PointedBoneBlock::isFluidLiquid).orElse(Fluids.EMPTY);
    }

    private static Optional<PointedBoneBlock.DrippingFluid> getFluid(World world, BlockPos pos, BlockState state) {
        return !isPointingDown(state) ? Optional.empty() : getSupportingPos(world, pos, state, 11).map(posx -> {
            ...
            return new PointedBoneBlock.DrippingFluid(blockPos, fluid, blockState);
        });
    }
    ...
    private static boolean canGrow(BlockState dripstoneBlockState, BlockState waterState) {
        return dripstoneBlockState.isOf(Blocks.BONE_BLOCK) && waterState.isOf(Blocks.WATER) && waterState.getFluidState().isStill();
    }
