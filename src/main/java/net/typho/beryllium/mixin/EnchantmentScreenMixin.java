    ...
    public void drawBackground(DrawContext context, float delta, int mouseX, int mouseY) {
        ...
        for (int id = 0; id < 3; id++) {
            ...
            if (power == 0) {
                ...
                if (enchOptional.isEmpty() || (lapis < id + 1 || client.player.experienceLevel < power) && !client.player.getAbilities().creativeMode || !Beryllium.hasEnoughCatalysts(catalystSlot, enchOptional.get(), handler.enchantmentLevel[id], client.player)) {
                    RenderSystem.enableBlend();
                    context.drawGuiTexture(ENCHANTMENT_SLOT_DISABLED_TEXTURE, m, y + 14 + 19 * id, 108, 19);
        ...
        for (int j = 0; j < 3; j++) {
            ...
            if (!optional.isEmpty()) {
                ...
                if (this.isPointWithinBounds(60, 14 + 19 * j, 108, 17, mouseX, mouseY) && k > 0 && l >= 0 && optional != null) {
                    ...
                    ItemStack catalyst = Beryllium.getEnchantmentCatalyst(optional.get(), l);
                    if (!bl) {
                        ...
                        if (this.client.player.experienceLevel < k) {
                            ...
                            list.add(Text.literal(catalyst.getCount() + "x ").append(catalyst.getName()).formatted(Beryllium.hasEnoughCatalysts(handler.getSlot(2).getStack(), optional.get(), l, client.player) ? Formatting.GRAY : Formatting.RED));
